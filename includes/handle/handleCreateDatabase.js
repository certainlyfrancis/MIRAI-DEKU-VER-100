...
module.exports = function ({ Users, Threads, Currencies }) {
    ...
    return async function ({ event }) {
        ...
        var random2 = job[Math.floor(Math.random() * job.length)]
                ...
                logger(global.getText('handleCreateDatabase', 'newThread',chalk.hex("#" + random)(`New group: `) + chalk.hex("#" + random1)(`${threadID}`) + "  ||  " + chalk.hex("#" + random2)(`${threadIn4.threadName}`)), '《 DEKU THREAD 》');
            }
            if (!allUserID.includes(senderID) || !userName.has(senderID)) {
                ...
                logger(global.getText('handleCreateDatabase', 'newUser', chalk.hex("#" + random)(`New users: `) + chalk.hex("#" + random1)(`${singleData.name}`) + " || " + chalk.hex("#" + random2)(`${senderID}`)), '《 DEKU USER 》');
            }
